...
Time threshold = Time(5000000); // 5ms instead of 5ms
Time interval = Time(55000000); // 55ms instead of 100ms
...
LteRlcUm::DoTransmitPdcpPdu (Ptr<Packet> p)
{
  ...
  if (cur_qdelay > threshold)
  {
    ...
    else if (Simulator::Now() - cur > interval)
    {
      ...
      if (count_new == 0)
      {
        // updating the drop count and drop time every time we drop a packet
        count_new += 1;
        next_drop_time = Simulator::Now().GetMilliSeconds() + interval.GetMilliSeconds() / sqrt(count_new);
        m_txDropTrace(p);  // Record the drop
        return;
      }
